...
void WebConfigServer::parseConfig(const JsonDocument& doc){
    ...
    device.angle_accuracy = doc["device"]["angle_accuracy"] | 10.0;
    device.servo_speed_ms = doc["device"]["servo_speed_ms/60"] | 300;
    device.loop_sensor_time_ms = doc["device"]["loop_sensor_time_ms"] | 100;
